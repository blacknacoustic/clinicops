...
export default function TaskBoard() {
  ...
  const loadData = async () => {
    // Explicitly ask for INTERNAL_TASK category
  ...
  useEffect(() => {
    if (patientSearch.length > 1 && !selectedPatient) {
      // In this setup, we'll search by unique names in your existing data
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      // If no patient selected, use default '1900-01-01'
      ...
      // Fix: Ensure empty string becomes null for the database
  ...
  return (
    <Protected>
      <div className="max-w-full mx-auto">
        ...
        {/* Task List - Only shows INTERNAL_TASK now */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          {/* ... existing table headers ... */}
          <div className="divide-y divide-slate-100">
            {tasks.map(task => (
              <div key={task.id} className="grid grid-cols-12 gap-4 p-5 items-center hover:bg-slate-50/50">
                <div className="col-span-5 font-bold text-slate-900">{task.patient_last_name}</div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden p-8 space-y-5">
              ...
              <form onSubmit={handleCreateTask} className="space-y-4">
                ...
                <textarea placeholder="Initial Comments" className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl h-24 font-medium"
